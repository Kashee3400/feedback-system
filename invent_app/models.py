...
class MPPs(models.Model):
    mcc = models.ForeignKey(MCCs, on_delete = models.CASCADE)
    mpp_loc = models.CharField(max_length = 100)
    mpp_loc_code = models.CharField(max_length = 20,primary_key=True,default = "EX_CODE")
    created_at = models.DateTimeField(auto_now_add=True,blank=True, null=True)
...
class Facilitator(models.Model):
    mcc = models.ForeignKey(MCCs, on_delete=models.SET_NULL, null=True)
    name = models.CharField(max_length=200)
    created_at = models.DateTimeField(auto_now_add=True,blank=True, null=True)
    
    def __str__(self):
        # Add leading zeros to the code attribute to ensure it has four digits
        # padded_code = self.code.zfill(4)
...
class Members(models.Model):
    mpp = models.ForeignKey(MPPs, on_delete=models.SET_NULL, null=True)
    name = models.CharField(max_length=200)
    ...
    def __str__(self):
        # Add leading zeros to the code attribute to ensure it has four digits
        padded_code = self.code.zfill(4)
        return f"{self.name} - {padded_code}"
