...
class AnimalBreed(models.Model):
    ...
    animal_type = models.ForeignKey(
        AnimalType, on_delete=models.CASCADE, related_name="breeds"
    )
    created_at = models.DateTimeField(auto_now=True)
    sync = models.BooleanField(default=False)
...
class Cattle(models.Model):
    ...
    farmer = models.ForeignKey(
        Member, on_delete=models.CASCADE, related_name="farmer_animals"
    )
    breed = models.ForeignKey(
        AnimalBreed, on_delete=models.CASCADE, related_name="breed_animals"
    )
    tag_type = models.ForeignKey(
        TAGType,
        on_delete=models.CASCADE,
        related_name="tag_animals",
        null=True,
        blank=True,
    )
...
class CattleTagging(models.Model):
    cattle = models.OneToOneField(
        Cattle, on_delete=models.CASCADE, related_name="cattle_tagged"
    )
    tag_number = models.CharField(max_length=20)
    ...
    def __str__(self):
        return (
            f"{self.cattle} Tag Number: {self.tag_number} - Date: {self.tagging_date}"
        )
...
class MedicineStock(models.Model):
    disease = models.ForeignKey(
        Disease, on_delete=models.CASCADE, related_name="disease_medicines"
    )
...
class DoctorMedicineStock(models.Model):
    doctor = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, related_name="medicine_stocks"
    )
...
class CaseEntry(models.Model):
    animal = models.ForeignKey(
        Cattle, on_delete=models.CASCADE, related_name="cattle_cases"
    )
    applied_by_ext = models.ForeignKey(
        CustomUser,
        on_delete=models.SET_NULL,
        null=True,
        related_name="case_by_ext",
        blank=True,
    )
    applied_by_member = models.ForeignKey(
        Member, on_delete=models.CASCADE, related_name="member_cases"
    )
    status = models.ForeignKey(
        CattleCaseStatus, on_delete=models.SET_NULL, blank=True, null=True
    )
    ...
    def __str__(self):
        appliedby = (
            self.applied_by_ext if self.applied_by_ext else self.applied_by_member
        )
...
class AnimalDiagnosis(models.Model):
    case_entry = models.ForeignKey(
        CaseEntry, on_delete=models.CASCADE, related_name="case_entry_diagnosis"
    )
    disease = models.ForeignKey(Disease, on_delete=models.SET_NULL, null=True)
    status = models.ForeignKey(
        DiagnosisCattleStatus, on_delete=models.SET_NULL, null=True
    )
...
class DiagnosisSymptoms(models.Model):
    ...
    diagnosis = models.ForeignKey(
        AnimalDiagnosis, on_delete=models.CASCADE, related_name="diagnosis_symptoms"
    )
    remark = models.TextField(blank=True, null=True)
    created_at = models.DateTimeField(auto_now=True)
    sync = models.BooleanField(default=False)
...
class CasePayment(models.Model):
    case_entry = models.ForeignKey(
        CaseEntry, on_delete=models.CASCADE, related_name="payments"
    )
