    ...
    def post(self, request, format=None):
        ...
        for member_code in member_codes:
            try:
                ...
                if VCGMemberAttendance.objects.filter(
                    member=vcg_group_mem, meeting=meeting
                ).exists():
                    attendance = VCGMemberAttendance.objects.get(
                        member=vcg_group_mem, meeting=meeting
                    )
                    attendance.status = "present"
                    attendance.save()
                else:
                    attendance = VCGMemberAttendance.objects.create(
                        member=vcg_group_mem, status="present", meeting=meeting
                    )
            except VMembers.DoesNotExist:
                return Response(
                    {"message": f"Member with code {member_code} not found"},
                ...
                return Response(
                    {
                        "message": f"VCGroup for member with code {member_code} not found"
                    },
                    status=status.HTTP_404_NOT_FOUND,
                )
        ...
        return Response(
            {"message": "Attendance Marked Successfully"}, status=status_code
        )
...
class ZeroDaysReasonReport(APIView):
    ...
    def post(self, request):
        ...
        meeting = VCGMeeting.objects.get(meeting_id=meeting_id)
        if ZeroDaysPouringReport.objects.filter(
            meeting=meeting, member=vmember
        ).exists():
            report = ZeroDaysPouringReport.objects.get(meeting=meeting, member=vmember)
            report.reason = reason
            report.save()
        else:
            report = ZeroDaysPouringReport.objects.create(
                member_id=member_id, reason_id=reason_id, meeting=meeting
            )
        response = {
...
class MonthAssignmentAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            mnrl_mxr_member = members.filter(
                mpp_id=mpp_code, mineral_bag__gt=0
            ).order_by("-mineral_bag")
            cattle_feed_member = members.filter(
                mpp_id=mpp_code, cattle_bag__gt=0
            ).order_by("-cattle_bag")
            response_data = asgnmnt_serializer.data
            member_serializer = VMembersSerializer(max_pouring_member, many=True)
            cattle_feed_member_serializer = VMembersSerializer(
                cattle_feed_member, many=True
            )
            ...
            return Response(
                {"message": "VMPP with the provided ID does not exist"},
                status=status.HTTP_404_NOT_FOUND,
            )
        except MonthAssignment.DoesNotExist:
            return Response(
                {"message": "MonthAssignment for the provided VMPP ID does not exist"},
                status=status.HTTP_404_NOT_FOUND,
            )
...
class ComplaintReport(APIView):
    ...
    def post(self, request):
        ...
        meeting = VCGMeeting.objects.get(meeting_id=meeting_id)
        if MemberComplaintReport.objects.filter(
            meeting=meeting, member=vmember
        ).exists():
            ...
            report = MemberComplaintReport.objects.create(
                member_id=member_id, reason_id=reason_id, meeting=meeting
            )
        response = {
            "message": "Submitted Successfully",
    ...
    def post(self, request):
        ...
        if meeting_id:
            try:
                ...
                return Response(
                    {"message": "Meeting with the specified ID does not exist"},
                    status=status.HTTP_404_NOT_FOUND,
                )
        else:
            return Response(
...
class DataCollectionWizard(SessionWizardView):
    ...
    def handle_non_pourer_meet_form(self, form):
        if self.request.POST.get("want_to_add_more"):
            try:
                ...
                messages.success(
                    self.request,
                    "Record added successfully. You can add more records now.",
                )
                return self.render(self.get_form("non_pourer_meet"))
            except Exception as e:
                messages.error(
                    self.request, f"An error occurred while saving the record: {str(e)}"
                )
                return self.render(form)
        else:
            if form.is_valid():
                return (
                    super().post()
                )  # Continue with the default behavior of the wizard
            else:
                messages.error(self.request, "Please correct the errors below.")
                return self.render(form)
    ...
    def handle_step(self, form, current_step):
        try:
            self.save_form_data(
                self.get_form_session(), current_step, form.cleaned_data
            )
            messages.success(
                self.request, f"Successfully completed step: {current_step}."
            )
            return super().post()
        except Exception as e:
            messages.error(
                self.request, f"An error occurred while processing the step: {str(e)}"
            )
            return self.render(form)
        ...
        try:
            with transaction.atomic():
                for form in form_list:
                    ...
                    instance.session = self.get_form_session()
                    instance.save()
                FormProgress.objects.filter(session=self.get_form_session()).update(
                    status="completed"
                )
            ...
            messages.error(
                self.request,
                "An error occurred while finalizing your data. Please try again.",
            )
...
class EventSessionListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == "export":
            return self.export_selected(request, selected_rows)
        elif action == "delete":
            return self.delete_selected(request, selected_rows)

    def delete_selected(self, request, selected_rows):
        queryset = EventSession.objects.filter(id__in=selected_rows)
        total = queryset.count()
        if not queryset.exists():
            messages.error(request, "No valid members found for deletion.")
            return redirect("event_session_list")
        queryset.delete()
        messages.success(
            request, f"Successfully deleted {total} members."
        )
        return redirect("event_session_list")

    def export_selected(self, request, selected_rows):
        if selected_rows:
            sessions = EventSession.objects.filter(id__in=selected_rows)
        else:
        ...
        for session in sessions:
            if session.mppvisit.exists():
                visit = session.mppvisit.last()
                data.append({
                    "MCC": visit.mcc,
                    "MCC Code": visit.mcc_code,
                    "MPP": visit.mpp_name,
                    "MPP Code": visit.mpp,
                    "MPP Visit By": visit.facilitator_name,
                    "Created At": session.created_at.strftime("%Y-%m-%d %I:%M %p"),
                    "Updated At": session.updated_at.strftime("%Y-%m-%d %I:%M %p"),
                })
        ...
        response['Content-Disposition'] = f'attachment; filename="{datetime.now().date()}selected_sessions.xlsx"'
...
class MppVisitReportView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        context = {
            "session": session,
            "maintenance_checklist_data": self.get_maintenance_checklist_data(session),
            "facilitator_data": self.get_facilitator_data(session),
            "composite_data_data": self.get_composite_data(session),
            "dispatch_data_data": self.get_dispatch_data(session),
            "non_pourer_meet_data": self.get_non_pourer_meet_data(session),
            "vcg_meeting_data": self.get_vcg_meeting_data(session),
            "membership_app_data": self.get_membership_app_data(session),
            "demands_data": self.get_demands_data(session),
        }
        return render(request, self.template_name, context)

    def get_maintenance_checklist_data(self, session):
        try:
    ...
    def get_facilitator_data(self, session):
        try:
    ...
    def get_composite_data(self, session):
        try:
    ...
    def get_dispatch_data(self, session):
        try:
    ...
    def get_non_pourer_meet_data(self, session):
        try:
    ...
    def get_vcg_meeting_data(self, session):
        try:
    ...
    def get_membership_app_data(self, session):
        try:
    ...
    def get_demands_data(self, session):
        try:
